#!/usr/bin/env python3
# SIRTS v8 – Top 100 | Pure scalp signals | 100% confirmation | CRT + Turtle + Wave + Volume + Bias
...
LOG_CSV = "./sirts_v8_top100_signals.csv"
...
def pos_size_units(entry, sl, conf_pct):
    risk_percent = 0.04 if conf_pct >= 100 else BASE_RISK
...
def btc_trend_agree():
    ...
    trend_by_sma = "bull" if (sma200 and btc_price > sma200) else ("bear" if sma200 else None)
...
def analyze_symbol(symbol):
    ...
    if tf_confirmations >= CONF_MIN_TFS and chosen_dir and chosen_entry is not None:
        ...
        if btc_agree is None:
            skipped_signals += 1
            print(f"Skipping {symbol}: BTC trend unknown.")
            return False
        if chosen_dir == "BUY" and btc_dir != "bull":
            skipped_signals += 1
            print(f"Skipping {symbol}: BTC direction mismatch for BUY.")
            return False
        if chosen_dir == "SELL" and btc_dir != "bear":
            skipped_signals += 1
            print(f"Skipping {symbol}: BTC direction mismatch for SELL.")
            return False
        ...
        sl, tp1, tp2, tp3 = trade_params(symbol, entry, chosen_dir)
        if not sl:
            skipped_signals += 1
            print(f"Skipping {symbol}: trade params fail.")
            return False

        units, margin, exposure, risk = pos_size_units(entry, sl, 100)

        confirmed_list = ", ".join(confirming_tfs)
        per_tf_text    = " | ".join([f"{tf} b{breakdown_per_tf[tf]['bull_score']}/r{breakdown_per_tf[tf]['bear_score']} bias:{breakdown_per_tf[tf]['bias']} vol_ok:{int(breakdown_per_tf[tf]['vol_ok'])}" for tf in confirming_tfs])
        ...
        header = (f"✅ {chosen_dir} {symbol} (100% CONF)\n"
                  ...
                  f"⚠ Risk used: {risk*100:.2f}% | Sentiment:{sentiment}")
        full_msg = f"{header}\n\n📊 Per‐TF: {per_tf_text}"

        send_message(full_msg)
        open_trades.append({"s":symbol,"side":chosen_dir,"entry":entry,"tp1":tp1,"tp2":tp2,"tp3":tp3,"sl":sl,"st":"open","units":units})
        signals_sent_total += 1
        print(f"✅ Signal sent for {symbol} at entry {entry}.")
        log_signal([datetime.utcnow().isoformat(), symbol, chosen_dir, entry, tp1, tp2, tp3, sl, chosen_tf, units, margin, exposure, risk*100, "open", per_tf_text])
        return True
    else:
        # Rough debug skip
        # print(f"⏭ No signal for {symbol}: only {tf_confirmations}/{len(TIMEFRAMES)} confirmed.")
        return False
...
print("✅ SIRTS v8 Top100 deployed.")
...
while True:
    try:
        ...
        for i, sym in enumerate(SYMBOLS, start=1):
            ...
            analyze_symbol(sym)
            time.sleep(0.3)  # small per‐symbol delay to manage rate limits
        check_trades()
